...
export default function NewMemberPage() {
  const [form, setForm] = useState({ firstName:'', lastName:'', idNumber:'', phone:'', email:'', province:'Gauteng', membershipTier:'standard', ageVerified:false, ficaConsent:false })
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!form.ageVerified || !form.ficaConsent) { setError('You must confirm age verification and FICA consent.'); return }
    setLoading(true)
    setError('')
  ...
  if (success) return (
    <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background:'#f8f9fa' }}>
      <div style={{ background:'white', borderRadius:'16px', padding:'48px', textAlign:'center', maxWidth:'400px', border:'0.5px solid #e5e7eb' }}>
        ...
        <div style={{ fontSize:'13px', color:'#6b7280', marginBottom:'24px' }}>{form.firstName} {form.lastName} has been added. FICA verification is pending.</div>
        <Link href="/members">
          <button style={{ background:'#1a7a4a', color:'white', border:'none', padding:'10px 24px', borderRadius:'8px', fontSize:'13px', fontWeight:'600', cursor:'pointer' }}>View Members →</button>
        </Link>
      </div>
    </div>
  )
      ...
      <div style={{ maxWidth:'600px', margin:'0 auto' }}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ display:'flex', gap:'10px', justifyContent:'flex-end' }}>
            <Link href="/members">
              <button type="button" style={{ padding:'10px 20px', border:'1px solid #d1d5db', borderRadius:'8px', fontSize:'13px', background:'white', cursor:'pointer' }}>Cancel</button>
            </Link>
